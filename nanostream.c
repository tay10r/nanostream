...
static void
to_eigen_values(const float* v, float* eigen_values)
{
...
static int
quantize(const float x, const float min_x, const float max_x, const int res)
{
...
static void
quantize_eigen_values(const float* eigen_values, const float* ev_min, const float* ev_max, unsigned char* bits)
{
  ...
  bits[2] =
    (quantize(eigen_values[2], ev_min[2], ev_max[2], 15) << 4) | quantize(eigen_values[3], ev_min[3], ev_max[3], 255);
